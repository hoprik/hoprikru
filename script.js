...
function logger(){
	info.innerHTML = "xyz "+Math.round(controls.getObject().position.x)+" "+Math.round(controls.getObject().position.y)+" "+Math.round(controls.getObject().position.z)+" direction "+getDirection(camera.getWorldDirection(new THREE.Vector3()))
}
...
function genRays(numRays){
	...
	for (let i = 0; i < numRays; i++) {
		const angle = (i / numRays) * Math.PI * 2; // Угол в радианах
		const direction = new THREE.Vector3(Math.cos(angle), 0, Math.sin(angle)); // Создание вектора направления
		rayDirections.push(direction); // Добавляем вектор направления в массив
	}
